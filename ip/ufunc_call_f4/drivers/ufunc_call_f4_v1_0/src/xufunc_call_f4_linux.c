...
typedef struct {
    ...
    XUfunc_call_f4_uio_map maps[ MAX_UIO_MAPS ];
} XUfunc_call_f4_uio_info;

/***************** Variable Definitions **************************************/
static XUfunc_call_f4_uio_info uio_info;
...
static int uio_info_read_name(XUfunc_call_f4_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XUfunc_call_f4_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XUfunc_call_f4_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XUfunc_call_f4_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XUfunc_call_f4_Initialize(XUfunc_call_f4 *InstancePtr, const char* InstanceName) {
	XUfunc_call_f4_uio_info *InfoPtr = &uio_info;
