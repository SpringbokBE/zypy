...
void mmul_v1(hls::stream<T_SIDE> &AB, hls::stream<T_SIDE> &C, ap_uint<6> l, ap_uint<6> m, ap_uint<6> n)
{
	...
	C1: for (int j = 0; j < n; ++j)
		C2: for (int i = 0; i < l; ++i) {
			#pragma HLS PIPELINE II=1
			T sum = 0;
			C3: for (int k = 0; k < m; ++k) {
				sum += a[i][k] * b[k][j];
			}
			C << push_stream<T, T_SIDE>(sum, i == (l - 1) and j == (n - 1));
		}
}
