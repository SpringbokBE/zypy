...
// Custom type to hold an int along with AXI-Stream side-channel information.
...
typedef struct {
	unsigned char reduce_op;	// Reduction operation to use.
	unsigned char element_op; 	// Element-wise operation to use.
	unsigned char final_op;		// Element-wise operation to do on the result.
} packed_t;
