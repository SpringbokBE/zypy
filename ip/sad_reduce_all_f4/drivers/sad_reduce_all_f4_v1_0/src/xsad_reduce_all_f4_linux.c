...
typedef struct {
    ...
    XSad_reduce_all_f4_uio_map maps[ MAX_UIO_MAPS ];
} XSad_reduce_all_f4_uio_info;

/***************** Variable Definitions **************************************/
static XSad_reduce_all_f4_uio_info uio_info;
...
static int uio_info_read_name(XSad_reduce_all_f4_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSad_reduce_all_f4_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSad_reduce_all_f4_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSad_reduce_all_f4_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSad_reduce_all_f4_Initialize(XSad_reduce_all_f4 *InstancePtr, const char* InstanceName) {
	XSad_reduce_all_f4_uio_info *InfoPtr = &uio_info;
